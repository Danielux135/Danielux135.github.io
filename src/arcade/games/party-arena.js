...
/* ==========================================================================
   JUEGO 8 — BEAT BLITZ (toca en el beat exacto: pura precisión rítmica)
...
Arcade.register({
    ...
    createSession(api, run) {
        const map = run.map;
        const events = run.events; // onsets de la canción

        // ventanas de timing en segundos
        const CFG = {
        ...
        // multiplier de combo: x1 → x2 → x4 → x8 cada 8 aciertos
        const comboMult = () => combo >= 24 ? 8 : combo >= 16 ? 4 : combo >= 8 ? 2 : 1;

        // los beats de la canción (usamos la rejilla de beats)
        const beats = [];
        {
            ...
            while (cur < (map.dur || 300)) {
                beats.push({ t: cur, state: 'pending' }); // pending→hit/miss/ignored
                cur += beatT;
            }
        }
        let bIdx = 0; // índice del primer beat que aún puede ser tocado

        // feedback flotante en canvas (texto + posición + vida)
        const floats = [];
        ...
        function doTap() {
            ...
            // busca el beat más cercano que no haya sido juzgado
            ...
            if (bestIdx === -1) {
                // toque fuera de ventana: pierde combo
                combo = 0;
                blip(200, 0.05, 0.08);
                haptic(30);
        ...
        // radio del orbe central pulsante
        let pulseR = 0;
        // destello verde al acertar en perfect
        let flashT = 0;
        ...
        return {
            frame(dt) {
                ...
                // marcar beats que ya pasaron como miss
                ...
                // anillo exterior de anticipación: el beat más cercano hacia el futuro
                let nextBeat = null;
                ...
                if (nextBeat) {
                    ...
                    if (until > 0 && until < lead * 1.5) {
                        // anillo que se cierra hacia el orbe
                    ...
                    if (Math.abs(now - nextBeat.t) <= CFG.winF) {
                        // dentro de la ventana: halo verde intenso
                ...
                // indicador de ventana: arco interior que muestra la precisión disponible
                ...
                // hint de tecla (primeros 3 segundos)
                if (t < 3.5) {
